...
export default async function Layout({
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden p-2">
      ...
      <div className="relative z-10 flex h-full flex-col overflow-hidden rounded-2xl shadow-sm lg:flex-row">
        <AppMainSidebar userName={session.name} />
        <main className="flex min-w-0 flex-1 overflow-hidden px-1.5">
          <div className="min-w-0 flex-1 overflow-auto no-scrollbar rounded-3xl px-2 pb-2">
            {children}
          </div>
        </main>
      </div>
